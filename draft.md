...
来自 `utils.input_setup` 和 `train.py`（commit `3e123e59e7517205318acf0a49eb126d045f7312`）。训练时每张源图/掩码都按 stride=24 滑窗裁成 128×128 patch，并用 `(imread(...) - 127.5)/127.5` 归一化到 [-1,1]。  
...
```
- 对齐注释：`config.image_size=128`、`config.stride=24`（见 5.5），对应论文训练细节“crop 128×128 with stride 24 得到 6921 patch”与“输入归一化到 [-1,1]”，且无下采样保持尺寸。
...
class STDFusionNet:
    ...
    def ir_feature_extraction_network(self, ir_image):
        with tf.compat.v1.variable_scope('ir_extraction_network'):
            # 结构与 vi_extraction_network 完全相同，作用域独立确保参数不共享
            with tf.compat.v1.variable_scope('conv1'):
                weights = tf.compat.v1.get_variable("w", [5, 5, 1, 16],
                                                    initializer=tf.truncated_normal_initializer(stddev=1e-3))
                bias = tf.compat.v1.get_variable("b", [16], initializer=tf.constant_initializer(0.0))
...
- 对齐注释：5×5 Conv+LeakyReLU 是 Fig.3 的 common layer；三个 ResBlocks 对应 Fig.3 中的重复模块；两分支同构不同参量体现 pseudosiamese。重建网络 4×ResBlock 后接 Tanh；`concat` 对应 Fig.3 的通道拼接；所有卷积 padding=SAME、stride=1 确保无下采样。

### 5.2 ResBlock 结构（对应 Fig.3 底部局部结构）
...
with tf.compat.v1.variable_scope('block2'):
    ...
    block2_output = tf.nn.leaky_relu(conv3 + identity_conv)
```
- 代码来源：`train_network.py` block2（红外、可见、重建分支结构同型）。
- 对齐注释：主分支 1×1→3×3→1×1 且 Conv1/Conv2 后接 leaky ReLU，旁路 identity 1×1 升维；`conv3 + identity_conv` 后再过 leaky ReLU，对应 Fig.3 ResBlock 的“+”与激活。

### 5.3 Sobel 梯度算子（对应式(4)(5)中的 ∇）
...
- 对齐注释：`filter1/2` 为 Sobel 水平/垂直核，对应论文“we employ the Sobel operator”；输出 `Gradient` 即式(4)(5) 中的 $\nabla I$。

### 5.4 Loss 计算（对应式(2)-(6)，含 mask 分区）
...
```
- 代码来源：`model.py`（commit 同上）。
- 对齐注释：`(mask+1)/2` 恢复二值显著区域；`1 - ir_mask` 是背景 mask（论文“inverted”）；四项对应式(2)–(5) 的显著/背景像素与梯度 L1，`reduce_mean` 实现 $\frac{1}{HW}\|\cdot\|_1$。`g_loss_2` 中显著区域乘 $\alpha=7$，背景系数为 1，对应式(6)。
...
```
- 代码来源：`train.py`（超参）与 `model.py`（训练循环），commit 同上。
- 对齐注释：`epoch=30`、`batch_size=32`、`learning_rate=1e-3`、`stride=24`、`image_size=128` 完全对齐论文训练设置；Adam 优化器对应论文“TensorFlow + Adam”。训练集为 TNO 20 对图像，经 5.1 裁剪共 6921 对 patch；测试阶段不裁剪且不输入 mask。
